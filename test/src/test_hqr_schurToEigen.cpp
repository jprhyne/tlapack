...
TEMPLATE_TEST_CASE("schur form is backwards stable", "[hqr][schur]", TLAPACK_REAL_TYPES_TO_TEST)
{
    ...
    idx_t low = (matrix_type == "Inner Window") ? n/4 : 0;
    idx_t igh = (matrix_type == "Inner Window") ? 3*n/4 : n - 1;

    // zero out columns 0 to low - 1
    for (idx_t j = 0; j < low; j++) {
        for (idx_t i = j+1; i < n; i++) {
            if (i == j) continue;
            A(i,j) = zeroT;
        }
    }
    for (idx_t i = igh + 1; i < n; i++) {
        for (idx_t j = 0; j < n; j++) {
            if (i > j)
                A(i,j) = zeroT;
        }
