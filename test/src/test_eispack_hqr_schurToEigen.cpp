...
TEMPLATE_TEST_CASE("schur form is backwards stable", "[hqr][schur]", TLAPACK_REAL_TYPES_TO_TEST)
{
    ...
    retCode = eispack_hqr_schurToEigen(H, 0, n - 1, wr, wi, Z, norm);
    CHECK(retCode == 0);
    ...
    if (testingScheme == "Complex Multiplication") {
        ...
        gemm(Op::NoTrans, Op::NoTrans, real_t(1), firstCheck, Zi, real_t(-1), firstCheck);
        // Compute the frobenius norm of the residual
        real_t normR = lange(tlapack::frob_norm, firstCheck);
