...
TEMPLATE_TEST_CASE("schur form is backwards stable", "[hqr][schur]", TLAPACK_REAL_TYPES_TO_TEST)
{
    ...
    */
    ...
    int retCode = tlapack::eispack_hqr(H, ilo, igh, wr, wi, true, eye, norm);
    CHECK(retCode == 0);
    ...
    auto orthZ = check_orthogonality(eye,res);
    ...
    auto sim_res_norm = check_similarity_transform(U,eye,H);
    CHECK(sim_res_norm <= tol * normA);
    // Now, we check if we can propogate the transformations properly
    std::vector<T> tmp_; auto tmp = new_matrix( tmp_, n, n);
    std::vector<T> temp_; auto temp = new_matrix( temp_, n, n);

    gemm(Op::NoTrans, Op::NoTrans, real_t(1), Q, eye, real_t(0), tmp);
    orthZ = check_orthogonality(tmp,res);
    
    CHECK(orthZ <= tol);
    
    gemm(Op::NoTrans, Op::NoTrans, real_t(1), tmp, H, real_t(0), res);

    lacpy(Uplo::General, A, temp);
    gemm(Op::NoTrans, Op::Trans, real_t(1), res, tmp, real_t(-1), temp);
